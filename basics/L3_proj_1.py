...
def open_and_close():
        ...
        for interface in ni.interfaces()[1:]:   #For interfaces eth0 and wlan0
            ...
            try:
                    ...
                    pass
        camera_input = 'http://' + ip + ':8090/?action=stream'   # Address for stream
                ...
                if image is not None:
                    ...
                    if len(cnts) and len(cnts) < 3:                             # If more than 0 and less than 3 closed shapes exist
                        ...
                        while (len(cnts)>1):
                            ...
                            angle = 1*round(((center[0]/width)-0.5)*fov, 3)           # angle of vector towards target center from camera, where 0 deg is centered
                            ...
                            if abs(e_width) < width_margin:                                 
                                print("target acquired, moving in")
                                ...
                                wheel_speed = L2_inverse_kinematics.getPdTargets(np.array([0.75*fwd_effort, -0.25*angle]))   # Find wheel speeds for approach and heading correction
                                L2_speed_control.driveClosedLoop(wheel_speed, wheel_measured, de_dt)  # Drive closed loop
                                print("tracking angle= ", angle)
                                print("want phil = ",wheel_speed[0],"want phir = ", wheel_speed[1])
                                print('tracking phil= ', wheel_measured[0] , 'tracking phir =  ', wheel_measured[1])
                                continue
                            if abs(angle) < angle_margin:
                                wheel_speed = L2_inverse_kinematics.getPdTargets(np.array([0.1, 0.75*angle]))    # Find wheel speeds for only turning
                                print("tunring angle= ", angle)
                                print("want phil = ",wheel_speed[0],"want phir = ", wheel_speed[1])
                                print('tracking phil= ', wheel_measured[0] , 'tracking phir =  ', wheel_measured[1])
                                L2_speed_control.driveClosedLoop(wheel_speed, wheel_measured, 0)          # Drive robot
                                continue     # Drive robot
